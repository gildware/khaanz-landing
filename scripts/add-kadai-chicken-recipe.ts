/**
 * Add Kadai Chicken half + full recipes (kitchen recipe sheet).
 * Run: npx tsx scripts/add-kadai-chicken-recipe.ts
 */
import { getPrisma } from "../src/lib/prisma";

const MENU_ITEM_ID = "kadai-chicken";
const BOILED_CHICKEN_ID = "boiled-chicken";
const INDIAN_GRAVY_ID = "indian-gravy";
...
const HALF_LINES: RecipeLine[] = [
  ...
  { inventoryItemId: "cmrnf1omg006ppm0ibm5xfpti", qtyBase: 50, note: "Onion" },
  { inventoryItemId: "cmrnf5c1g006spm0i0gufi4tl", qtyBase: 25, note: "Capsicum" },
  {
    componentMenuItemId: INDIAN_GRAVY_ID,
    componentVariationId: INDIAN_GRAVY_ID,
    qtyBase: 250,
    note: "Indian Gravy",
  },
...
const FULL_LINES: RecipeLine[] = [
  ...
  { inventoryItemId: "cmrnf1omg006ppm0ibm5xfpti", qtyBase: 70, note: "Onion" },
  { inventoryItemId: "cmrnf5c1g006spm0i0gufi4tl", qtyBase: 35, note: "Capsicum" },
  {
    componentMenuItemId: INDIAN_GRAVY_ID,
    componentVariationId: INDIAN_GRAVY_ID,
    qtyBase: 450,
  ...
  { inventoryItemId: "cmrncbtds0041pm0ifuzdyb56", qtyBase: 4, note: "Red Chilli Powder" },
...
const RECIPES = [
  { variationId: "kadai-chicken-half", label: "Kadai Chicken — Half", lines: HALF_LINES },
  { variationId: "kadai-chicken-full", label: "Kadai Chicken — Full", lines: FULL_LINES },
] as const;
...
async function main() {
  ...
  for (const prepId of [BOILED_CHICKEN_ID, INDIAN_GRAVY_ID]) {
    const prep = await prisma.menuItem.findUnique({ where: { id: prepId } });
    if (!prep) {
      console.error(`Prep item missing: ${prepId}`);
      process.exit(1);
    }
  }

...
export async function deleteMenuWastage(
  tx: Prisma.TransactionClient,
  menuWastageEntryId: string,
): Promise<void> {
  const entry = await tx.menuWastageEntry.findUnique({
    where: { id: menuWastageEntryId },
    select: { id: true },
  });
  if (!entry) throw new Error("WASTAGE_NOT_FOUND");

  const children = await tx.wastageEntry.findMany({
    where: { menuWastageEntryId },
    select: { id: true },
  });
  for (const child of children) {
    await reverseIngredientWastageStock(tx, child.id);
  }
  await tx.wastageEntry.deleteMany({ where: { menuWastageEntryId } });
  await tx.menuWastageEntry.delete({ where: { id: menuWastageEntryId } });
}

export async function updateMenuWastage(
  tx: Prisma.TransactionClient,
  menuWastageEntryId: string,
  input: {
    menuItemId: string;
  ...
  const existing = await tx.menuWastageEntry.findUnique({
    where: { id: menuWastageEntryId },
    select: { id: true },
  });
  if (!existing) throw new Error("WASTAGE_NOT_FOUND");
  ...
  await tx.menuWastageEntry.update({
    where: { id: menuWastageEntryId },
    data: {
      menuItemId: input.menuItemId,
      variationId: input.variationId,
  ...
  return { id: menuWastageEntryId, ingredientWastageCount };
}
